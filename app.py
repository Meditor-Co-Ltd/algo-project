...
def calculate_absorbance_features(measure, reference, dark, cal_data=None):
    """Calculate absorbance features for one sample"""
    try:
        ...
        # Features from cal_data if available
...
def predict_glucose(spectral_data):
    ...
    try:
        ...
        cal_data = spectral_data.get('cal_data', None)
        
        # Validate input data
        if not measure or not reference or not dark:
            return None
        ...
        logger.debug(f"Processing spectral data with {len(measure)} points")
...
def predict():
    ...
    try:
        ...
        required_fields = ['measure', 'reference', 'dark']
